                    ...
                    <div v-else>
                      ...
                      <div class="row">
                        ...
                        <div class="col-md-6">
                          <p>
                            <strong
                              >Lieu accouchement après consentement:</strong
                            >
                            {{
  ...
  methods: {
    ...
    async getGrossesse() {
      try {
        ...
        this.rendezVous.sort(
          (a, b) => new Date(a.date_rv) - new Date(b.date_rv)
        );
      } catch (error) {
        console.error(
    ...
    handleConseilTableAction({ action, row }) {
      switch (action) {
        case ACTION_EDIT:
          this.editConseil(row);
          break;
        case ACTION_DELETE:
          this.deleteConseil(row.id);
          break;
        default:
          break;
      }
    ...
    async editConseil(conseil) {
      console.log(conseil.id);

      try {
        await conseilService.updateConseil(conseil.id, this.conseil);
        Swal.fire(
          "Modifié !",
        ...
        Swal.fire(
          "Erreur !",
          "Il y a eu un problème lors de la modification du conseil.",
          "error"
        );
      }
    },

    async deleteConseil(conseilId) {
      ...
      if (result.isConfirmed) {
        try {
          await conseilService.deleteConseil(conseilId); // Appelez le service pour supprimer le conseil
          Swal.fire(
            "Supprimé !",
            "Le conseil a été supprimé avec succès.",
            "success"
          );
          this.getGrossesse(); // Rechargez les conseils après suppression
        } catch (error) {
          Swal.fire({
            title: "Erreur !",
            text: "Il y a eu un problème lors de la suppression du conseil.",
            icon: "error",
          });
        }
      }
